...
// Main ProductDetail Component that accepts either a product or a promise of a product
export function ProductDetailStreamable({
  product,
}: {
  product: Streamable<Product>;
}) {
  // If no product exists yet (loading state)
  if (!product) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
      ...
      {/* Image Gallery with Suspense */}
      <Stream fallback={<ImageGallerySkeleton />} value={product}>
        {(resolvedProduct) => <ProductGallery product={resolvedProduct} />}
      </Stream>

      {/* Product Info with Suspense */}
      <div className="lg:col-span-2 space-y-6">
        <Stream fallback={<ProductInfoSkeleton />} value={product}>
          {(resolvedProduct) => <ProductInfo product={resolvedProduct} />}
        </Stream>
      </div>
    </div>
  );
