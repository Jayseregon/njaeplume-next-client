...
jest.mock('next-intl', () => ({
  useTranslations: () => (key: string) => {
    const translations: Record<string, string> = {
      'title': 'Shop',
      'pageIntro': 'Discover our collection of digital resources to enhance your creativity',
...
jest.mock('@/actions/prisma/action', () => ({
  getLatestProductsByCategory: (limit: number) => mockGetLatestProductsByCategory(limit),
}));
...
function MockShopPage() {
  return (
    <div>
      ...
      <p>Discover our collection of digital resources to enhance your creativity</p>
...
describe('ShopPage', () => {
  ...
  it('renders the page title and intro text', async () => {
    ...
    expect(screen.getByText('Discover our collection of digital resources to enhance your creativity')).toBeInTheDocument();
  });
  ...
  it('handles empty categories with coming soon message', async () => {
    ...
    expect(screen.getByTestId('empty-state')).toHaveTextContent('Products coming soon...');
  });
});
