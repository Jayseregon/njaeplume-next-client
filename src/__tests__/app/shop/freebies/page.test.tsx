...
jest.mock('@/components/product/CategoryGallery', () => ({
  CategoryGallery: ({ products, showAsFreebies }: { products: any[], showAsFreebies?: boolean }) => (
    <div 
...
jest.mock('@/actions/prisma/action', () => ({
  getProductsByCategory: (category: string) => mockGetProductsByCategory(category),
}));
...
describe('FreebiesPage', () => {
  ...
  it('renders the page title correctly', async () => {
    ...
    render(<FreebiesPage />);
    expect(screen.getByTestId('page-title')).toHaveTextContent('Free Downloads');
  });
  ...
  it('renders loading state initially with empty products', async () => {
    ...
    expect(screen.getByTestId('category-gallery')).toHaveTextContent('No products found');
  ...
  it('displays freebies correctly when loaded', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('product-prod_1')).toHaveTextContent('Free Watercolor Brush');
      expect(screen.getByTestId('product-prod_2')).toHaveTextContent('Free Sticker Pack');
    });
  });
