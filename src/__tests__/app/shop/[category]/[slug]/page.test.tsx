...
import ProductPage, { generateMetadata } from '@/app/shop/[category]/[slug]/page';
...
const mockProduct = {
  ...
  name: 'Sample Product',
  price: 29.99,
  description: 'A sample product description that is long enough for testing metadata.',
...
describe('Dynamic Product Page', () => {
  ...
  describe('Page Component', () => {
    it('renders the product detail for valid category and slug', async () => {
      ...
      expect(screen.getByTestId('product-name')).toHaveTextContent('Sample Product');
      expect(screen.getByTestId('product-price')).toHaveTextContent('$29.99');
      expect(screen.getByTestId('product-category')).toHaveTextContent('brushes');
  ...
  describe('generateMetadata', () => {
    ...
    it('truncates description to 160 characters for SEO', async () => {
      ...
      const longDesc = 'This is a very long description that exceeds 160 characters. '.repeat(5);
      (getProductBySlug as jest.Mock).mockResolvedValue({
        ...mockProduct,
