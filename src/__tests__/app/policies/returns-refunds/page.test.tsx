...
const returnsTranslations = {
  ...
  policy: {
    ...
    noRefunds:
      "Due to the digital nature of our products, which can be immediately accessed and used upon purchase, all sales are final.",
    clarification:
      "We strongly encourage you to carefully review the product description, preview images, and any available demonstrations before making a purchase.",
    faultyFiles:
      "In the unlikely event that you experience issues with a downloaded file (e.g., it is corrupted or incomplete), please contact us immediately.",
    termsOfServiceLink:
      "Please also refer to our comprehensive Terms of Service for further details regarding your purchase.",
    governingLaw:
      "This Returns & Refunds policy is governed by and construed in accordance with the laws of the Province of Quebec.",
    questions:
      "We are happy to answer any questions regarding our products before your purchase to ensure they will meet your needs.",
    additionalQuestions:
      "If you have any additional questions or concerns, please feel free to contact us through our dedicated contact form.",
...
jest.mock("next-intl", () => ({
  useTranslations: () => (key: string) => {
    ...
    if (key.includes(".")) {
      ...
      const sectionData =
        returnsTranslations[section as keyof typeof returnsTranslations];
...
describe("Returns & Refunds Page", () => {
  ...
  it("renders policy content with all sections", () => {
    ...
    expect(
      screen.getByText(/Due to the digital nature of our products/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/We strongly encourage you to carefully review/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/In the unlikely event that you experience issues/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /Please also refer to our comprehensive Terms of Service/,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/This Returns & Refunds policy is governed by/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/We are happy to answer any questions/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/If you have any additional questions or concerns/),
    ).toBeInTheDocument();
  });
