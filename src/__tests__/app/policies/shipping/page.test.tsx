...
const shippingTranslations = {
  ...
  "policy": {
    "intro": "Thank you for purchasing our products at njaeplume.com operated by NJAE Plume G.P..",
    "noShipping": "Due to the digital nature of our products, we do not provide any shipping services, and no shipping fees are applicable.",
    "download": "Each product is available for direct download immediately upon purchase.",
    "technicalIssue": "In the event of a technical issue (for example, if you do not receive a download link or are unable to download your product), please contact us immediately through our dedicated contact form with your proof of purchase.",
    "understanding": "Thank you for your understanding.",
    "additionalQuestions": "If you have any additional questions, feel free to contact us through our dedicated contact form.",
...
jest.mock("next-intl", () => ({
  useTranslations: () => (key: string) => {
    ...
    if (key.includes(".")) {
      ...
      const sectionData = shippingTranslations[section as keyof typeof shippingTranslations];
      return (typeof sectionData === 'object' && sectionData !== null)
        ? (sectionData as Record<string, string>)[subKey] || key
        : key;
    }
    return shippingTranslations[key as keyof typeof shippingTranslations] || key;
  }
}));
...
describe("Shipping Page", () => {
  ...
  it("renders policy content with all sections", () => {
    ...
    expect(screen.getByText(/Thank you for purchasing our products/)).toBeInTheDocument();
    
    // Check key policy sections
    expect(screen.getByText(/Due to the digital nature of our products, we do not provide any shipping/)).toBeInTheDocument();
    expect(screen.getByText(/Each product is available for direct download/)).toBeInTheDocument();
    expect(screen.getByText(/In the event of a technical issue/)).toBeInTheDocument();
    expect(screen.getByText(/Thank you for your understanding/)).toBeInTheDocument();
    expect(screen.getByText(/If you have any additional questions/)).toBeInTheDocument();
  });
