    ...
    items: [
      {
        ...
        product: {
          ...
          category: { id: 'cat_2', name: 'Stickers' },
...
describe('OrdersPage', () => {
  ...
  it('shows back to dashboard button', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('link--account')).toHaveAttribute('href', '/account');
    });
  });
  ...
  it('handles successful checkout and clears cart when session_id is present', async () => {
    ...
    await waitFor(() => {
      ...
      expect(toast.success).toHaveBeenCalledWith('Payment successful! Your order is complete.');
      
      // Verify URL was updated to remove the session_id
      expect(mockReplace).toHaveBeenCalled();
      
      // Verify console.log was called with the expected message
      expect(console.log).toHaveBeenCalledWith('Checkout successful, clearing cart...');
    });
  });
  ...
  it('handles API errors gracefully', async () => {
    ...
    await waitFor(() => {
      expect(toast.error).toHaveBeenCalledWith('Failed to load your order history.');
